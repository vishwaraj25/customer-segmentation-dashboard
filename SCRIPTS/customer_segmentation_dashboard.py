...
if selected_graph == "Purchase Frequency Distribution":
    ...
    st.plotly_chart(fig)
    st.write("Looking at the graph, we observe that the majority of customers purchase infrequently, suggesting a large portion of one-time buyers.")
    ...
    if all(col in df.columns for col in ["Recency", "Monetary", "KMeans_Cluster"]):
        ...
        st.error("⚠️ Required columns (Recency, Monetary, KMeans_Cluster) not found in dataset.")
    ...
    if all(col in df.columns for col in ["Recency", "Frequency", "KMeans_Cluster"]):
        ...
        st.error("⚠️ Required columns (Recency, Frequency, KMeans_Cluster) not found in dataset.")
    ...
    if all(col in df.columns for col in ["Recency", "Frequency"]):
        ...
        st.error("⚠️ Required columns (Recency, Frequency) not found in dataset.")
    ...
    if all(col in df.columns for col in ["Monetary", "Frequency"]):
        ...
        st.error("⚠️ Required columns (Monetary, Frequency) not found in dataset.")
    ...
    if "RFM_Score" in df.columns:
        ...
        st.error("⚠️ Required column (RFM_Score) not found in dataset.")
    ...
    if "KMeans_Cluster" in df.columns:
        ...
        st.error("⚠️ Required column (KMeans_Cluster) not found in dataset.")

elif selected_graph == "Final Summary":
    st.header("📌 Final Summary: Key Insights")

    st.subheader("1️⃣ Purchase Frequency Distribution")
    st.write("- The majority of customers purchase infrequently, indicating a large portion of one-time buyers.")
    st.write("- A small percentage of customers purchase regularly, representing loyal shoppers.")

    st.subheader("2️⃣ Customer Segmentation (KMeans & Hierarchical Clustering)")
    ...
    st.subheader("3️⃣ Recency vs Frequency Analysis")
    ...
    st.subheader("4️⃣ Monetary vs Frequency Analysis")
    ...
    st.subheader("5️⃣ RFM Score Distribution")
    ...
    st.subheader("6️⃣ Cluster Size Comparison")
